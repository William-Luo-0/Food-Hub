<?php
    if (isset($_GET["ID"])) {
        ...
        $sql = "SELECT customer.Name AS CustomerName,
            ...
            INNER JOIN restaurant ON restaurant.UID=foodorder.RestaurantUID
            INNER JOIN customer ON customer.UID=foodorder.CustomerUID 
            INNER JOIN address ON customer.AID=address.AID
            INNER JOIN postalcode ON address.PostalCode=postalcode.PostalCode
            WHERE foodorder.OID=? AND foodorder.";
        $UID = 0;
        $OrderDate = 0;
        $isFulfilled = true;
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            echo '<div class="container card mt-4">
                    <div class="order-template">
                        ...
                        <h3 style="text-align: center; text: #ffffff;"><a class="plain-anchor" href="../restaurant.php?ID='.$result["UID"].'">Restaurant: '.$result["RestaurantName"].'</a></h3>
            ...
            if ($result["OrderDuration"] < 0) {
                ...
                $timeFormat = sprintf('%02d:%02d:%02d', $hours, $mins, $secs);
                        ...
                        <div class="row">
                            <div class="col"><p class="lead">Name: '.$result["CustomerName"].'</p></div>
                        </div>
                        <div class="row">
                        ...
                        <table class="table" id="order-items">
                            ...
                            <tbody>';
            $sql = "SELECT foodorderitems.UID, foodorderitems.Amount, menuitem.Name, menuitem.Price, menuitem.FoodType
                    FROM foodorderitems 
                    INNER JOIN menuitem ON foodorderitems.UID=menuitem.UID AND foodorderitems.IID=menuitem.IID 
                    WHERE OID=?";
            $stmt = mysqli_stmt_init($mysqli);
            $totalprice = 0.00;
            if (!mysqli_stmt_prepare($stmt, $sql)) {
                ...
                while ($rows = mysqli_fetch_assoc($results)) {
                    ...
                    ';
                    $orderNumber += 1;
                    $totalprice += $rows["Price"] * $rows["Amount"];
                };
            }
            ...
            if ($_SESSION["AccountType"] == "Restaurant" && $_SESSION["UID"] == $UID && !$isFulfilled) {
                echo '<div class="py-4">
                        <form action="includes/fulfillorder.inc.php" method="post">
                            <div class="form-group">
                                <input type="text" class="form-control" name="Type" value="Restaurant" hidden>
                            </div>
                            <div class="form-group">
